Usage BUILD-OCLP-Mod testing on macOS Tahoe 26 and macOS Sequoia 15.6

### Prerequisite: Command Line Tools (CLT) for Xcode
- from `xcode-select —install`
- Or Developer Apple Command Line Tools
- If you have `Xcode installed` that's also good
  

  
### [x] `Clone BUILD-OCLP-Mod.command:`
```bash
git clone https://github.com/chris1111/BUILD-OCLP-Mod.git && ./BUILD-OCLP-Mod/Pics/SetIcon/seticon -d ./BUILD-OCLP-Mod/Pics/SetIcon/SetIcon/AppIcon.icns ./BUILD-OCLP-Mod/BUILD-OCLP-Mod.command && Open ./BUILD-OCLP-Mod
```


### BUILD-OCLP-Mod.command: ⬇︎
<img src="Pics/Screenshot-1.png" alt="Github Project" style="width:80%;">

### Options: A ⬇︎
<img src="Pics/Screenshot-2.png" alt="Github Project" style="width:80%;">

### Options: D ⬇︎
<img src="Pics/Screenshot-3.png" alt="Github Project" style="width:80%;">

### Install OCLP-Mod: ⬇︎
<img src="Pics/Screenshot-4.png" alt="Github Project" style="width:80%;">
